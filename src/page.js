...
const redirectTarget=()=>{const q=new URLSearchParams(location.search).get('redirect');if(!q)return null;try{const r=new URL(q,location.href);if(r.protocol==='https:'&&(r.hostname==='desksuite.cloud'||r.hostname.endsWith('.desksuite.cloud')))return r.href;}catch(e){}return null;};
const goAfterLogin=()=>{const t=redirectTarget();if(t){location.replace(t);}else{location.reload();}};
...
async function passkeyLogin(msgEl){
  try{
    ...
    if(v.ok){goAfterLogin();}else{say(msgEl,v.error||'Accesso fallito.');}
...
function viewLogin(hasPasskey){
  ...
  $('go').onclick=async()=>{
    ...
    if(v.ok){goAfterLogin();}else{say($('m'),v.error||'Errore.');}
  };
...
(async()=>{
  try{
    ...
    if(s.authenticated){const t=redirectTarget();if(t){location.replace(t);return;}viewHome(s.email);}
